...
!pip -q install yfinance pandas numpy ta requests beautifulsoup4 lxml html5lib openpyxl
...
SEND_LINE = False  # ← LINE送信するときは True に変更

LINE_SEND_MODE = "broadcast"
# "broadcast"：公式アカウントの友だち全員に送信
# "push"：指定したUSER_IDに送信

LINE_CHANNEL_ACCESS_TOKEN = "ここにチャネルアクセストークンを入力"
LINE_USER_ID = "ここにUSER_IDを入力"  # pushの場合のみ必要
...
def send_line_message(message):
    if not SEND_LINE:
        print("LINE送信はOFFです。SEND_LINE=True にすると送信されます。")
        return

    if LINE_CHANNEL_ACCESS_TOKEN == "ここにチャネルアクセストークンを入力":
        print("LINE_CHANNEL_ACCESS_TOKENが未入力です。")
        return
    ...
    if LINE_SEND_MODE == "broadcast":
        ...
        if LINE_USER_ID == "ここにUSER_IDを入力":
            print("LINE_USER_IDが未入力です。")
            return
...
if ranking.empty:
    ...
    if buy_candidates.empty:
        ...
        display(buy_candidates.head(20))

    print("\n")
    print("=" * 80)
    print("半導体・AI・データセンター関連ランキング")
    print("=" * 80)

    semiconductor_ranking = ranking[ranking["半導体AI関連"] == "該当"]
    display(semiconductor_ranking.head(20))
    ...
    display(high_dividend_ranking.head(20))
